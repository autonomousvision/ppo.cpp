...
inline Tensor normalize_imagenet(const Tensor& rgb) {
    const Tensor out = rgb.clone() / 255.0;

    const auto options = TensorOptions().device(rgb.device()).dtype(rgb.dtype());
    const Tensor mean = torch::tensor({0.485, 0.456, 0.406}, options).view({1, 3, 1, 1});
    const Tensor std  = torch::tensor({0.229, 0.224, 0.225}, options).view({1, 3, 1, 1});
