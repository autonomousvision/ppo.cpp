...
class SensorAgentImpl final : public nn::Module {
  ResNetRL resnet{nullptr};
  nn::Sequential cnn{nullptr};
  nn::Sequential linear{nullptr};
  nn::Sequential state_linear{nullptr};
  nn::Sequential value_measurements_encoder{nullptr};
  nn::Sequential value_head{nullptr};
  nn::Sequential linear_alpha{nullptr};
  nn::Sequential linear_beta{nullptr};
...
public:
  explicit SensorAgentImpl(const GlobalCarlaConfig& config, const int action_dim, const float action_high, const float action_low):
  config_(config)
  {
    constexpr int num_input_channels = 3;
    int n_flatten;
    if (config_.image_architecture == "rl_resnet22") {
      resnet = ResNet22_PPO(num_input_channels);
      register_module("cnn", resnet);
      n_flatten = 512 * 6 * 18;
    }
    else {
      ...
      n_flatten = 256 * 2 * 2;  // Number of channels output from the CNN
    }
    ...
    state_linear = nn::Sequential(
        nn::Linear(config_.obs_num_measurements + 4, 256),
        nn::LayerNorm(nn::LayerNormOptions({256})),
        nn::ReLU(),
        nn::Linear(256, 256),
        nn::LayerNorm(nn::LayerNormOptions({256})),
    ...
    value_head = nn::Sequential(nn::Linear(512, 256),
                                nn::LayerNorm(nn::LayerNormOptions({256})),
                                nn::ReLU(),
                                nn::Linear(256, 256),
                                nn::LayerNorm(nn::LayerNormOptions({256})),
    ...
    value_measurements_encoder = nn::Sequential(nn::Linear(config_.num_value_measurements, 256),
                            nn::LayerNorm(nn::LayerNormOptions({256})),
                            nn::ReLU(),
                            nn::Linear(256, 256),
                            nn::LayerNorm(nn::LayerNormOptions({256})),
    ...
    register_module("value_measurements_encoder", value_measurements_encoder);

    linear_alpha = nn::Sequential(nn::Linear(n_features, action_dim));
    register_module("linear_alpha", linear_alpha);

    linear_beta = nn::Sequential(nn::Linear(n_features, action_dim));
    register_module("linear_beta", linear_beta);
  ...
  Tensor forward_cnn_encoder(const unordered_map<string, Tensor>& obs) {
    auto rgb = obs.at("rgb").to(kFloat32);

    const auto measurements = obs.at("measurements");
    const auto target_point = obs.at("target_point");
    const auto target_point_next = obs.at("target_point_next");
    Tensor merged_measurements = cat({measurements, target_point, target_point_next}, 1);

    // Visualization code
    // 1. Move to CPU and detach from the autograd graph (just in case)
    // auto img_tensor = rgb.detach().cpu();
    //
    // // Remove batch dimension if it exists (e.g., [1, C, H, W] -> [C, H, W])
    // if (img_tensor.ndimension() == 4) {
    //   img_tensor = img_tensor.index({0});
    // }
    //
    // // 2. Permute from [C, H, W] to [H, W, C] for OpenCV
    // img_tensor = img_tensor.permute({1, 2, 0});
    //
    // // 4. Convert to 8-bit unsigned integer and ensure memory is contiguous
    // img_tensor = img_tensor.to(torch::kU8).contiguous();
    //
    // // 5. Wrap the tensor data in a cv::Mat
    // // Note: tensor.size(0) is Height, tensor.size(1) is Width
    // cv::Mat cv_img(img_tensor.size(0), img_tensor.size(1), CV_8UC3, img_tensor.data_ptr<uchar>());
    //
    // // 6. Convert RGB (Libtorch default) to BGR (OpenCV default)
    // cv::cvtColor(cv_img, cv_img, cv::COLOR_RGB2BGR);
    //
    // // Display the image
    // cv::imshow("Libtorch Tensor Visualization", cv_img);
    ...
    Tensor x;
    if (config_.image_architecture == "rl_resnet22") {
      x = resnet->forward(rgb);
    }
    else {
      x = cnn->forward(rgb);
    }
    x = flatten(x, 1);
    Tensor latent_state = state_linear->forward(merged_measurements);
    x = cat({x, latent_state}, 1);
    x = linear->forward(x);
    return x;
  }

  Tensor get_value(const unordered_map<string, Tensor>& obs) {
    const auto value_measurements = obs.at("value_measurements");
    Tensor features = forward_cnn_encoder(obs);
    Tensor value_states = value_measurements_encoder->forward(value_measurements);
    Tensor value_features = cat({features, value_states}, 1);
  ...
  tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor> forward(const unordered_map<string, Tensor>& obs, Tensor actions=at::empty({0}), const string sample_type="sample"s,
    const optional<Generator>& generator=nullopt)
  {
    const auto bev_semantics = obs.at("bev_semantics");
    const auto measurements = obs.at("measurements");
    const auto value_measurements = obs.at("value_measurements");

    Tensor features = forward_cnn_encoder(obs);

    Tensor alpha = linear_alpha->forward(features);
    Tensor beta = linear_beta->forward(features);

    Tensor value_states = value_measurements_encoder->forward(value_measurements);
    Tensor value_features = cat({features, value_states}, 1);
    Tensor values = value_head->forward(value_features);

    alpha = nn::functional::softplus(alpha) + config_.beta_min_a_b_value;
    beta = nn::functional::softplus(beta) + config_.beta_min_a_b_value;
    const auto proba_distribution = Beta(alpha, beta);
    ...
    if (config_.debug)
    {
      ...
      const auto proba_distribution_visu = Beta(alpha.slice(0, 0, 1), beta.slice(0, 0, 1));
      visualize_model(proba_distribution_visu, bev_semantics, measurements, value_measurements, actions, values);
    }

    return make_tuple(actions, log_prob, entropy, values, alpha.detach(), beta.detach());
  }
  ...
  Tensor convert_action_to_control(const Tensor& action) {
    ...
    control.index_put_({0}, action.index({0}));

    if (action.accessor<float, 1>()[1] > 0.0f) {
      control.index_put_({1}, action.index({1}));
      control.accessor<float, 1>()[2] = 0.0f;
    }
    else {
      control.accessor<float, 1>()[1] = 0.0f;
      control.index_put_({2}, -action.index({1}));
    }
    return control;
  }
