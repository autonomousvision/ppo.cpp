...
public:
    ...
    [[nodiscard]] virtual float get_action_space_min() const = 0;
    [[nodiscard]] virtual float get_action_space_max() const = 0;
    [[nodiscard]] virtual GlobalCarlaConfig get_config() const = 0;
...
public:
    ...
    env_array_(env_array), clip_actions_(clip_actions), num_envs_(env_array.size())
    {
        ...
        const auto n_envs = static_cast<int64_t>(num_envs_);
        auto obs_space = env_array_[0]->get_observation_space();
        obs_["bev_semantics"] = torch::zeros({n_envs, obs_space[0], obs_space[1], obs_space[2]}, torch::kUInt8);
        obs_["measurements"] = torch::zeros({n_envs, obs_space[3]});
        obs_["value_measurements"] = torch::zeros({n_envs, obs_space[4]});
        if (env_array_[0]->get_config().use_sensorimotor == true) {
            obs_["rgb"] = torch::zeros({n_envs, obs_space[5], obs_space[6], obs_space[7]}, torch::kUInt8);
            obs_["lidar"] = torch::zeros({n_envs, obs_space[8], obs_space[9]}, torch::kUInt8);
            obs_["compass"] = torch::zeros({n_envs, obs_space[10]});
            obs_["speed"] = torch::zeros({n_envs, obs_space[11]});
            obs_["gps"] = torch::zeros({n_envs, obs_space[12]});
            obs_["target_point"] = torch::zeros({n_envs, obs_space[13]});
            obs_["target_point_next"] = torch::zeros({n_envs, obs_space[14]});
        }
        rewards_ = torch::zeros({n_envs});
        terminations_ = torch::zeros({n_envs});
        truncations_ = torch::zeros({n_envs});
    }
    ...
    tuple<unordered_map<string, Tensor>, Tensor, Tensor, Tensor, vector<optional<env_info>>> step(const Tensor& actions) {
        ...
        for (int i = 0; i < env_array_.size(); ++i) {
            ...
            if (termination == true or truncation == true) {
                if (!info.has_value()) {
                    info = env_info{}; // Ensure info exists
                }
                // TODO store final_observation (we don't use it currently)

                // -1 == do not reseed the environment
                next_obs = env_array_[i]->reset(-1);
            }
            // Write the observation to our batch (this is either the standard step obs, or the brand-new reset obs)
            for (const auto& [key, tensor] : next_obs) {
                obs_[key].index_put_({i}, tensor);
            }
        ...
        for (const auto& [key, tensor] : obs_) {
            obs_copy[key] = tensor.detach().clone();
        }
...
// TODO fix autoresets in other envs and test.
// Comparable to gymnasiums SyncVectorEnv.
// class PufferDriveEnvAutoreset {
// private:
...
//     Tensor terminations_;
//     Tensor truncations_;
//     optional<env_info> infos_;
//     bool autoreset_env_;
//     const bool clip_actions_;
//
// public:
//
//     PufferDriveEnvAutoreset(const shared_ptr<EnvironmentWrapper>& env_, const bool clip_actions):
//     env_(env_), clip_actions_(clip_actions)
//     {
//         autoreset_env_ = false;
//         infos_ = std::nullopt;
//
//         const auto obs_space_vec = env_->get_observation_space_vector();
//         obs_ = torch::zeros({obs_space_vec.at(0), obs_space_vec.at(1)});
//         rewards_ = torch::zeros({obs_space_vec.at(0)});
//         terminations_ = torch::zeros({obs_space_vec.at(0)});
//         truncations_ = torch::zeros({obs_space_vec.at(0)});
//     }
//
//     ~PufferDriveEnvAutoreset() = default;
//
//     Tensor reset(const int seed) {
//         torch::NoGradGuard no_grad;
//         const auto next_obs = env_->reset(seed);
//         obs_ = next_obs;
//         autoreset_env_ = false;
//         return obs_;
//     }
...
//     }
//     [[nodiscard]] vector<int> get_observation_space_vector() const {
//         return env_->get_observation_space_vector();
//     }
//     [[nodiscard]] int get_action_space() const {
...
//         Tensor clipped_actions;
//         if (clip_actions_) {
//              clipped_actions = torch::clamp(actions, env_->get_action_space_min(), env_->get_action_space_max());
//         }
//         else {
//             clipped_actions = actions;
//         }
//         // TODO do I need to copy data?
//         if (autoreset_env_) {
//             // -1 == do not reseed the environment
//             const auto next_obs = env_->reset(-1);
//             obs_ = next_obs;
//             rewards_.accessor<float,1>()[i] = 0.0f;
//             terminations_.accessor<float,1>()[i] = static_cast<float>(false);
//             truncations_.accessor<float,1>()[i] = static_cast<float>(false);
