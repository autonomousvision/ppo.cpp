...
#include <gymcpp/pufferdrive/drive_env.h>
...
public:
  ...
  int eval_seed = 111111111;  // negative values mean no seeding will take place.
  ...
  bool torch_deterministic = false;
  string exp_name_stem = "PufferDrive-v0"s;
  ...
  string train_device = "cpu"s;  // Options: cpu, gpu  // TODO fix pc cuda
...
int main(const int argc, const char** argv) {
  ...
  // at::globalContext().setBenchmarkCuDNN(!config.torch_deterministic);
  // TODO fix compiler warnings
  // Use tensorcores TF32
  ...
  vector<shared_ptr<PufferDriveEnv>> envs;

  Tensor observation_mean;
  Tensor observation_std;

  filesystem::path map_folder = basedir / "pufferdrive" / "assets" / "binaries";
  cout << "Loading folder: " << map_folder.string() << endl;
  for (int i = 0; i < config.num_envs_per_device; ++i)
  {
    envs.push_back(make_shared<PufferDriveEnv>(map_folder, config.render, config.seed+i));
  }

  // TODO estimate
  observation_mean = torch::zeros({envs[0]->get_observation_space().at(1)}, dtype(kFloat).requires_grad(false));
  observation_std = torch::ones({envs[0]->get_observation_space().at(1)}, dtype(kFloat).requires_grad(false));

  auto agent = Agent(envs[0]->get_observation_space().at(1), envs[0]->get_action_space(),
  ...
  vector<Tensor> logprobs;
  vector<Tensor> rewards;
  vector<Tensor> dones;
  vector<Tensor> values;
  vector<Tensor> returns;
  vector<Tensor> advantages;

  vector<Tensor> next_obs;
  vector<Tensor> next_done;

  for (int i=0; i<config.num_envs_per_device; ++i) {
    const auto tmp_obs_vec = envs.at(i)->get_observation_space();
    obs.push_back(torch::zeros({config.num_steps, tmp_obs_vec.at(0), tmp_obs_vec.at(1)}, collect_devices));
    actions.push_back(torch::zeros({config.num_steps, tmp_obs_vec.at(0), envs[0]->get_action_space()}, collect_devices));
    logprobs.push_back(torch::zeros({config.num_steps, tmp_obs_vec.at(0)}, collect_devices));
    rewards.push_back(torch::zeros({config.num_steps, tmp_obs_vec.at(0)}, collect_devices));
    dones.push_back(torch::zeros({config.num_steps, tmp_obs_vec.at(0)}, collect_devices));
    values.push_back(torch::zeros({config.num_steps, tmp_obs_vec.at(0)}, collect_devices));
    returns.push_back(torch::zeros({config.num_steps, tmp_obs_vec.at(0)}, collect_devices));
    advantages.push_back(torch::zeros({config.num_steps, tmp_obs_vec.at(0)}, collect_devices));

    next_obs.push_back(torch::zeros({tmp_obs_vec.at(0), envs[0]->get_observation_space().at(1)}, collect_devices));
    next_done.push_back(torch::zeros({tmp_obs_vec.at(0)}, collect_devices));
  }
  ...
  for (int i = 0; i < config.num_envs_per_device; ++i) {
    next_obs.at(i) = envs[i]->reset(-1).to(collect_devices);  // Already seeded at initialization
  ...
  for (auto iteration : tq::trange(config.num_iterations, (rank == 0))) {
    ...
    for (int i = 0; i < config.num_envs_per_device; ++i) {
      ...
      auto collect = [&, i] {
        ...
        for (step = 0; step < config.num_steps; ++step) { // No grad
          NoGradGuard no_grad;
          obs[i].index_put_({step}, next_obs[i]);
          dones[i].index_put_({step}, next_done[i]);

          Tensor action, logprob, entropy, value, done;
          tie(action, logprob, entropy, value) = agent->get_action_and_value(next_obs[i].to(collect_devices), at::empty({0}), "sample", agent_generators[i]);

          values[i].index_put_({step}, value.squeeze(1));
          actions[i].index_put_({step}, action);
          logprobs[i].index_put_({step}, logprob);
          ...
          if (i == 0 && config.estimate_mean_std) {
            obs_mean_std.push_back(env_next_obs.clone());
          }

          done = torch::logical_or(termination, truncation).to(kFloat32);
          rewards[i].index_put_({step}, reward.to(collect_devices));
          next_obs[i] = env_next_obs.to(collect_devices);
          next_done[i] = done.to(collect_devices);

          for (const auto& info : infos)
          {
            infos_vec.push_back(info);
          }
    ...
    { // No grad
      // TODO could be parallelized. Measure if significant
      NoGradGuard noGrad;
      vector<Tensor> lastgaelam;
      vector<Tensor> nextnonterminal(config.num_envs_per_device, at::empty({0}, collect_devices));
      vector<Tensor> nextvalues(config.num_envs_per_device, at::empty({0}, collect_devices));

      for (int i=0; i<config.num_envs_per_device; ++i)
      {
        const auto tmp_obs_vec = envs.at(i)->get_observation_space();
        const Tensor next_value = agent->get_value(next_obs.at(i)).flatten();
        lastgaelam.push_back(torch::zeros({tmp_obs_vec.at(0)}, collect_devices));
        for (int t = num_train_steps_per_env-1; t >= 0; --t) {
          if(t == (config.num_steps-1)) {
            nextnonterminal.at(i) = 1.0f - next_done.at(i);
            nextvalues.at(i) = next_value;
          }
          else {
            nextnonterminal.at(i) = 1.0 - dones.at(i).index({t + 1});
            nextvalues.at(i) = values.at(i).index({t + 1});
          }
          Tensor delta = rewards.at(i).index({t}) + config.gamma * nextvalues.at(i) * nextnonterminal.at(i) - values.at(i).index({t});
          advantages.at(i).index({t}) = delta + config.gamma * config.gae_lambda * nextnonterminal.at(i) * lastgaelam.at(i);
          lastgaelam.at(i) = advantages.at(i).index({t});
        }
        returns.at(i) = advantages.at(i) + values.at(i);
      }
    }
    ...
    tt.tic();
    // TODO change preemption trick num_train_steps_per_env
    Tensor b_obs = cat(obs, 1).index({Slice(None, num_train_steps_per_env)}).reshape({-1, envs[0]->get_observation_space().at(1)}).to(train_devices);
    Tensor b_logprobs  = cat(logprobs, 1).index({Slice(None, num_train_steps_per_env)}).reshape({-1}).to(train_devices);
    Tensor b_actions = cat(actions, 1).index({Slice(None, num_train_steps_per_env)}).reshape({-1, envs[0]->get_action_space()}).to(train_devices);
    Tensor b_advantages = cat(advantages, 1).index({Slice(None, num_train_steps_per_env)}).reshape(-1).to(train_devices);
    Tensor b_returns = cat(returns, 1).index({Slice(None, num_train_steps_per_env)}).reshape(-1).to(train_devices);
    Tensor b_values = cat(values, 1).index({Slice(None, num_train_steps_per_env)}).reshape(-1).to(train_devices);
    long num_samples_collected = b_returns.sizes()[0];  // TODO aggregate across GPUs as this will differ


    // NOTE: We count duplicated samples from dd_ppo_preempt as as collected samples
    global_step += num_samples_collected;  // TODO multi-gpu
    ...
    // TODO multi-gpu
    int current_minibatch = num_samples_collected / config.num_minibatches;  // Intetionally rounds down // TODO make max value
    // Optimize network
    for (int epoch = 0; epoch < config.update_epochs; ++epoch) {
      ...
      for (int start = 0; start < num_samples_collected; start+=current_minibatch) { // TODO multi-gpu config.batch_size_per_device, config.minibatch_per_device
        int end = start + current_minibatch;
  ...
  { // no grad
    ...
    cout << "Training finished Press Enter to evaluate model..." << endl;
    ...
    for (int i = 0; i < config.num_eval_runs; ++i)
    {
      auto eval_env = make_shared<PufferDriveEnv>(map_folder, "human", config.eval_seed + i);
      auto next_obs_eval = eval_env->reset(-1);

      vector<float> episodic_returns_one_env;

      int num_vehicles = eval_env->get_observation_space().at(0);
      while (episodic_returns_one_env.size() < num_vehicles)
      {
        Tensor action;
        {
        ...
        auto [env_next_obs, reward, termination, truncation, infos] = eval_env->step(action.to(kCPU));
        next_obs_eval = env_next_obs;
        for (const auto info : infos)
        {
          auto [r, l, t] = info;
          cout << (boost::format("Evaluation result: episode=%d episodic_return=%.2f \n") % episodic_returns.size() % r).str();
          episodic_returns_one_env.push_back(r);
        }
      }
      episodic_returns.insert(episodic_returns.end(), episodic_returns_one_env.begin(), episodic_returns_one_env.end());
    }
