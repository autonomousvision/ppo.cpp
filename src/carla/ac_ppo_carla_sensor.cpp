// AC_PPO for sensorimotor agent. Currently work in progress.
...
shared_ptr<EnvironmentWrapperCarla> make_env(const shared_ptr<EnvironmentCarla>& env_0, const GlobalCarlaConfig& config) {
...
void save_state(const SensorAgent& agent, const optim::Adam& optimizer, const filesystem::path& folder, const string& model_file, const string& optimizer_file, GlobalCarlaConfig global_config) {
  NoGradGuard no_grad;
...
int main(int argc, char** argv) {
  ...
  at::globalContext().setAllowTF32CuBLAS(config.allow_tf32);
  at::globalContext().setAllowTF32CuDNN(config.allow_tf32);
  at::globalContext().setDeterministicCuDNN(config.torch_deterministic);
  at::globalContext().setBenchmarkCuDNN(config.benchmark);
  at::globalContext().setDeterministicAlgorithms(config.torch_deterministic, true);
  at::globalContext().setFloat32MatmulPrecision(config.matmul_precision);
  ...
  auto agent = SensorAgent(config, envs[0]->get_action_space(), envs[0]->get_action_space_max(), envs[0]->get_action_space_min());
  ...
  // TODO keep obs in RAM
  // We have much more RAM than VRAM, so we keep the observations in RAM to increase mini-batch size.
  auto obs_device = Device(kCPU);
  // Storage setup
  unordered_map<string, Tensor> obs;
  obs["bev_semantics"]      = torch::zeros({config.num_steps, config.num_envs_per_proc, obs_space[0], obs_space[1], obs_space[2]}, TensorOptions().dtype(kUInt8).device(obs_device));
  obs["measurements"]       = torch::zeros({config.num_steps, config.num_envs_per_proc, obs_space[3]}, obs_device);
  obs["value_measurements"] = torch::zeros({config.num_steps, config.num_envs_per_proc, obs_space[4]}, obs_device);
  obs["rgb"]                = torch::zeros({config.num_steps, config.num_envs_per_proc, obs_space[5], obs_space[6], obs_space[7]}, TensorOptions().dtype(kUInt8).device(obs_device));
  obs["lidar"]              = torch::zeros({config.num_steps, config.num_envs_per_proc, obs_space[8], obs_space[9]}, TensorOptions().dtype(kUInt8).device(obs_device));
  obs["compass"]            = torch::zeros({config.num_steps, config.num_envs_per_proc, obs_space[10]}, obs_device);
  obs["speed"]              = torch::zeros({config.num_steps, config.num_envs_per_proc, obs_space[11]}, obs_device);
  obs["gps"]                = torch::zeros({config.num_steps, config.num_envs_per_proc, obs_space[12]}, obs_device);
  obs["target_point"]       = torch::zeros({config.num_steps, config.num_envs_per_proc, obs_space[13]}, obs_device);
  obs["target_point_next"]  = torch::zeros({config.num_steps, config.num_envs_per_proc, obs_space[14]}, obs_device);
  ...
  next_obs["bev_semantics"]      = torch::zeros({config.num_envs_per_proc, obs_space[0], obs_space[1], obs_space[2]}, TensorOptions().dtype(kUInt8).device(obs_device));
  next_obs["measurements"]       = torch::zeros({config.num_envs_per_proc, obs_space[3]}, obs_device);
  next_obs["value_measurements"] = torch::zeros({config.num_envs_per_proc, obs_space[4]}, obs_device);
  next_obs["rgb"]                = torch::zeros({config.num_envs_per_proc, obs_space[5], obs_space[6], obs_space[7]}, TensorOptions().dtype(kUInt8).device(obs_device));
  next_obs["lidar"]              = torch::zeros({config.num_envs_per_proc, obs_space[8], obs_space[9]}, TensorOptions().dtype(kUInt8).device(obs_device));
  next_obs["compass"]            = torch::zeros({config.num_envs_per_proc, obs_space[10]}, obs_device);
  next_obs["speed"]              = torch::zeros({config.num_envs_per_proc, obs_space[11]}, obs_device);
  next_obs["gps"]                = torch::zeros({config.num_envs_per_proc, obs_space[12]}, obs_device);
  next_obs["target_point"]       = torch::zeros({config.num_envs_per_proc, obs_space[13]}, obs_device);
  next_obs["target_point_next"]  = torch::zeros({config.num_envs_per_proc, obs_space[14]}, obs_device);
  ...
  for (int i = 0; i < config.num_envs_per_proc; ++i) {
    ...
    for (const auto& [key, tensor] : reset_obs) {
      next_obs[key].index_put_({i}, tensor.squeeze(0).to(obs_device, false, false));
    }
  ...
  for (auto iteration : tq::trange(start_step, config.num_iterations, (rank == 0))) {
    ...
    for (int i = 0; i < config.num_envs_per_proc; ++i) {
      ...
      auto collect = [&, i] {
        ...
        for (step = 0; step < config.num_steps; ++step) { // No grad
          NoGradGuard no_grad;
          // TODO write to local memory and copy back later
          unordered_map<string, Tensor> agent_next_obs;
          for (const auto& [key, tensor] : next_obs) {
            obs[key].index_put_({step, i}, tensor.index({i}));
            agent_next_obs[key] = tensor.index({i}).unsqueeze(0).to(collect_devices, false, false);
          }
          ...
          // 1.8ms
          tie(action, logprob, entropy, value, mu, sigma) = agent->forward(agent_next_obs, at::empty({0}), "sample"s, agent_generators[i]);
          ...
          auto x = action.to(kCPU, false, false);
          // if (rank == 0 and i == 0) {
          //   ti.tic();
          // }
          // 20 ms
          auto [env_next_obs, reward, termination, truncation, infos] = envs[i]->step(x);
          // if (rank == 0 and i == 0) {
          //   ti.toc((boost::format("Rank: %d, Env: %d After step %d :") % rank % i % step).str());
          // }
          ...
          for (const auto& [key, tensor] : env_next_obs) {
            next_obs[key].index_put_({i}, tensor.index({0}).to(obs_device, false, false));
          }
    ...
    { // No grad
      ...
      unordered_map<string, Tensor> agent_next_obs;
      for (const auto& [key, tensor] : next_obs) {
        agent_next_obs[key] = tensor.to(collect_devices, false, false);
      }
      const Tensor next_value = agent->get_value(agent_next_obs).flatten();
      ...
      // TODO this could be the bug if preemption, envs that have longer rollouts have no advantages?
      for (int t = num_train_steps_per_env-1; t >= 0; --t) {
        if(t == (config.num_steps-1)) {  // TODO bug. Uses num_steps instead of num_train_steps_per_env
    ...
    // Not sent to train device
    b_obs["bev_semantics"]      = obs["bev_semantics"]     .index({Slice(None, num_train_steps_per_env)}).reshape({-1, obs_space[0], obs_space[1], obs_space[2]});
    ...
    b_obs["rgb"]                = obs["rgb"]               .index({Slice(None, num_train_steps_per_env)}).reshape({-1, obs_space[5], obs_space[6], obs_space[7]}).to(train_devices, false, false);
    b_obs["lidar"]              = obs["lidar"]             .index({Slice(None, num_train_steps_per_env)}).reshape({-1, obs_space[8], obs_space[9]}).to(train_devices, false, false);
    b_obs["compass"]            = obs["compass"]           .index({Slice(None, num_train_steps_per_env)}).reshape({-1, obs_space[10]}).to(train_devices, false, false);
    b_obs["speed"]              = obs["speed"]             .index({Slice(None, num_train_steps_per_env)}).reshape({-1, obs_space[11]}).to(train_devices, false, false);
    b_obs["gps"]                = obs["gps"]               .index({Slice(None, num_train_steps_per_env)}).reshape({-1, obs_space[12]}).to(train_devices, false, false);
    b_obs["target_point"]       = obs["target_point"]      .index({Slice(None, num_train_steps_per_env)}).reshape({-1, obs_space[13]}).to(train_devices, false, false);
    b_obs["target_point_next"]  = obs["target_point_next"] .index({Slice(None, num_train_steps_per_env)}).reshape({-1, obs_space[14]}).to(train_devices, false, false);
    ...
    for (int epoch = 0; epoch < config.update_epochs; ++epoch) {
      ...
      for (int start = 0; start < config.batch_size_per_device; start+=config.minibatch_per_device) {
        ...
        unordered_map<string, Tensor> b_obs_sampled;
        for (const auto& [key, tensor] : b_obs) {
          b_obs_sampled[key] = tensor.index({mb_inds}).to(train_devices);
        }

        auto [action, newlogprob, entropy, newvalue, mu, sigma] = agent->forward(b_obs_sampled, b_actions.index({mb_inds}));
    ...
    te.toc((boost::format("Rank: %d Time for entire iteration.") % rank).str());
    cout << std::flush;
  }
  if (rank == 0) {
